...
"""
左移横扫 - 使用AprilTag检测
持续左移并检测指定Tag，找到目标ID后返回

用法:
  python sweep_left.py <target_ids> [exclude_ids]
  示例:
    python sweep_left.py 3,4         # 左移横扫，找id=3或4
    python sweep_left.py 3,4 1,2     # 左移横扫，找id=3或4，排除id=1,2
"""
import sys
import time
import cv2
...
SWEEP_TIMEOUT = 40      # 最大扫描时间(秒)


def sweep_left(target_ids=None, exclude_ids=None, robot_name=ROBOT_NAME):
    """
    左移横扫，持续左移并检测Tag
    ...
    print(f'[*] [左横扫] 启动, 目标ID={target_ids}, 排除ID={exclude_ids}')
        ...
        while time.time() - start_time < SWEEP_TIMEOUT:
            ...
            for tag in tags:
                ...
                if is_target and not is_exclude:
                    print(f'[+] [左横扫] 找到 ID={tag.tag_id} (横移{sweep_count}次)')
                    try:
...
def main():
    # 解析命令行参数
    # 用法: sweep_left.py <target_ids> [exclude_ids]
    # 示例: sweep_left.py 3,4 1,2
    if len(sys.argv) < 2:
        print('用法: python sweep_left.py <target_ids> [exclude_ids]')
        print('示例: python sweep_left.py 3,4       # 找id=3或4')
        print('示例: python sweep_left.py 3,4 1,2   # 找id=3或4，排除id=1,2')
        sys.exit(1)
    ...
    result = sweep_left(target_ids=target_ids, exclude_ids=exclude_ids)
